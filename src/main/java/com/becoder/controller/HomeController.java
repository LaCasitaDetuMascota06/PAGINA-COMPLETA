...
public class HomeController {
	...
	public String saveUser(@ModelAttribute User user, HttpSession session, Model m, HttpServletRequest request) {
		...
		if (u != null) { // System.out.println("save sucess");
			session.setAttribute("msg", "Registro Completado");

		} else { // System.out.println("error in server");
			session.setAttribute("msg", "Vaya, algo a salido mal. :( )");
		}
	...
	public String verifyAccount(@Param("code") String code, Model m) {
		...
		if (f) {
			m.addAttribute("msg", "Felicidades!! Su cuenta a sido verificada");
		} else {
			m.addAttribute("msg", "Puede ser que su código de verificación sea incorrecto o ya esté verificado.");
		}
